...
{
  return home()->GetValue(static_cast<unsigned long>(idx()), col());
}


bool pqxx::field::is_null() const throw ()
{
  return home()->GetIsNull(static_cast<unsigned long>(idx()), col());
}


pqxx::field::size_type pqxx::field::size() const throw ()
{
  return home()->GetLength(static_cast<unsigned long>(idx()), col());
}
